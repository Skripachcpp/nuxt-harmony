...
export function treeParse(tree, level, parentPre, treeStr) {
  if (!level) level = 0;
  ...
  tree.forEach(function (child, index) {
    ...
    treeStr?.push("".concat(setPre(level, hasNext, parentPre)).concat(child.color ? chalk.hex(child.color)(child.name) : stringToColor(child.name)(child.name)));
    if (children) {
      treeParse(children, level + 1, setTransferPre(parentPre, hasNext), treeStr);
    }
  });
  return treeStr;
}
function check(tree, level) {
  if (typeof tree !== "object") return false;
  if (level >= 1000) return false;
  return true;
}
function setPre(level, hasNext, parentPre) {
  return "".concat(parentPre || "").concat(hasNext ? "├" : "└", "\u2500\u2500 ");
}
function setTransferPre(parentPre, hasNext) {
  return "".concat(parentPre || "").concat(hasNext ? "│" : " ", "   ");
}
// #endregion
...
async function readComponents(pathFull) {
  const files = findFiles(pathFull).filter((a) => a.includes(".tsx") && !a.includes(".test.") && !a.includes("._test."));
  const cmpByPath = {};
  // читаем файлы
  const readTasks = [];
...
async function initImports(cmpByPath) {
  const readTasks = [];
    ...
    for (const importMatched of importsMatched) {
      ...
      if (importPath.includes("bottom-child"))
        if (importPathArray?.[0] == "~") {
          ...
          if (importPathArray?.[1] != "components" && importPathArray?.[1] != "shared") {
            continue;
          }
...
function pathToRelativePath(path) {
...
async function main() {
  ...
  if (args.help) {
    ...
    console.log("3. на компоненты в components на уровень 0 или 1 если это файлы а не папка");
    console.log("4. все на кого ссылаются pages должны лежать в корневой папке components");
    console.log();
  ...
  await Promise.all([initImports(pagesCmpByPath), initImports(componentsCmpByPath)]);
  const errorAccumulator = new Set();
  function initChildren(cmpElement, level = 0, parentsAll = []) {
    ...
    if (parentsAll.includes(cmpElement)) {
      ...
      errorAccumulator.add(
        [color0("ошибка циклические зависимости"), color0(prnts.map((parent) => pathToRelativePath(parent.pathFull)).join(" => ")), color1(pathToRelativePath(cmpElement.pathFull))].join(" ")
      );
      return;
    }
    ...
    if (!cmpElement.parentLevels.includes(level)) cmpElement.parentLevels.push(level);
  ...
  function initParents(cmpElement, level = 0, parentsAll = []) {
    parentsAll = [...parentsAll];
    if (parentsAll.includes(cmpElement)) {
      const color0 = chalk.hex("#CD5C5C");
      const color1 = chalk.hex("#FF2400");
  ...
  const reverseTree = {};
  function initReverseTree(cmpElement, level = 0, parentsAll = []) {
    parentsAll = [...parentsAll];
    if (parentsAll.includes(cmpElement)) {
      const color0 = chalk.hex("#CD5C5C");
      const color1 = chalk.hex("#FF2400");
  ...
  let consoleTree = [];
  function mapTsxElement2ConsoleTree(tsxElement, level = 0) {
    if (tsxElement == null) return [];
    if (level > 4) return [];
    const consoleTreeElements = [];
    for (const chd of tsxElement.children || []) {
      if (!chd?.pathFull) continue;
      let name = pathToComponentName(chd.pathFull);
      if (args.paths) name = chd.pathFull.replace(__dirname + "/", "").replace("/index.tsx", "");
      const element = {
        name,
  ...
  const cmpUsed = {};
  function initCmpUsed(cmpElement, parentsAll = []) {
    parentsAll = [...parentsAll];
    if (parentsAll.includes(cmpElement)) {
      const color0 = chalk.hex("#CD5C5C");
      const color1 = chalk.hex("#FF2400");
  ...
  for (const pathFull in cmpNotUsed) {
    ...
    console.log(color0("не используется"), color1([pathFull.replace(__dirname + "/", "")].join(" ")));
  }
  console.log();
  function checkPaths(componentElement) {
    if (!componentElement?.parent?.length) return;
    const componentParentsSet = new Set();
    ...
    const componentParents = [...componentParentsSet].map((a) => a.split("/").filter((a) => a != "index.tsx"));
    let componentPath = componentElement.pathFull.replace(__dirname + "/", "").split("/");
    ...
    if (componentPath.length <= 2) {
      ...
      if (!componentElement.parent.some((parentElement) => !parentElement.pathFull.replace(__dirname + "/", "").startsWith("components/"))) {
        // то можно проверять можно ли положить его пониже
        const componentElementRelativePath = componentElement.pathFull.replace(__dirname + "/", "");
        const relativePaths = [];
        for (const parentElement of componentElement.parent || []) {
          const relativePath = path.dirname(parentElement.pathFull.replace(__dirname + "/", "")).split("/");
        ...
        if (canMove && !(componentElementRelativePath.startsWith("shared/") && relativePath[0] === "components")) {
          const color0 = chalk.hex("#CD5C5C");
          const color1 = chalk.hex("#FF2400");
          console.log(color0("перенести "), color1(componentElementRelativePath), color0("в"), color1(relativePaths?.[0].join("/")));
          anyChanges = true;
        }
      }
      return;
    ...
    for (const componentParent of componentParents) {
      if (componentParent.length > componentPath.length) {
        ...
        const folderParent = componentParent.filter((a, i) => i < componentParent.length - 1);
        const folderComponent = componentPath.filter((a, i) => i < componentParent.length - 1);
        if (folderParent.join("/") != folderComponent.join("/")) {
          const color0 = chalk.hex("#CD5C5C");
          const color1 = chalk.hex("#FF2400");
          console.log(color0("родитель "), color1(componentParent.join("/")), color0("в другом подкаталоге, чем зависимость"), color1(componentPath.join("/")));
          anyChanges = true;
        }
      }
    }
  ...
  for (const componentFullPath in pagesCmpByPath) {
    ...
    for (const children of componentElement.children || []) {
      const relativePath = children.pathFull.replace(__dirname + "/", "").replace("/index.tsx", "");
      ...
      if (relativePathArray.length > 2) {
        const color0 = chalk.hex("#CD5C5C");
        const color1 = chalk.hex("#FF2400");
        console.log(color0("page"), color1(pathToComponentName(componentFullPath)), color0("не может использовать компонент не верхнего уровня"), color1(relativePath));
      }
    }
  }
  let count = 0;
