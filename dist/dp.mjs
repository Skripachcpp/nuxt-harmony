...
async function main() {
    ...
    function initChildren(cmpElement, level = 0, parentsAll = []) {
        ...
        if (cmpElement == null)
            return;
        if (cmpElement.parentLevels == null)
            cmpElement.parentLevels = [];
    ...
    function initCmpUsed(cmpElement, parentsAll = []) {
        ...
        if (cmpElement == null)
            return;
        if (!cmpElement.pathFull)
            return;
        cmpUsed[cmpElement.pathFull] = true;
        for (const children of cmpElement.children || []) {
            if (!children?.pathFull)
                continue;
    ...
    for (const pathFull in componentsCmpByPath) {
        if (!cmpUsed[pathFull]) {
            ...
            for (const imp of componentsCmpByPath[pathFull].imports ?? []) {
                ...
                if (impCmp == null)
                    allParents[imp.pathFull + "/index.tsx"] = true;
                else
                    allParents[imp.pathFull ?? ""] = true;
            }
        }
    }
    ...
    function checkPaths(componentElement) {
        if (!componentElement?.parent?.length)
            return;
        ...
        if (componentPath.length <= 2) {
            ...
            if (!componentElement.parent.some((parentElement) => !parentElement.pathFull
                .replace(__dirname + "/", "")
